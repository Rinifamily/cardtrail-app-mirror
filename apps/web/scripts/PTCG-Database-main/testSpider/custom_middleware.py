...
class SeleniumMiddleware(object):
    ...
    def __init__(self, max_requests_per_driver=1000000):
        ...
        self.proxy_validator = ProxyValidator() if self.proxy_enabled else None
        self.proxy_list = []
    ...
    def from_crawler(cls, crawler):
        ...
        # Only load proxies if enabled
        if middleware.proxy_enabled:
            middleware.proxy_list = middleware.load_proxies()
            middleware.logger.info(f"Loaded {len(middleware.proxy_list)} proxies")
        else:
            middleware.logger.info("Proxy rotation disabled")
            middleware.proxy_list = []
    ...
    def process_request(self, request, spider):
        ...
        # Check if browser restart is requested (e.g., for each new set)
        restart_browser = request.meta.get('restart_browser', False)
        if restart_browser and self.driver:
            self.logger.info("🔄 Restarting browser for new set...")
            try:
                self.driver.quit()
            except:
                pass
            self.driver = None
            # Wait for Chrome process to fully exit before starting new one
            time.sleep(3)
    ...
    def driver_return(self):
        ...
        version_env = os.getenv('CHROME_VERSION_MAIN') or os.getenv('GOOGLE_CHROME_VERSION')
        if version_env:
            digits = ''.join(ch for ch in version_env if ch.isdigit())
            if digits:
                driver_kwargs["version_main"] = int(digits)
                self.logger.info(f"Using Chrome driver version_main={driver_kwargs['version_main']}")
