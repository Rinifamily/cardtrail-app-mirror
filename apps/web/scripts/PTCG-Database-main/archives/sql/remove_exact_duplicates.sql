-- Remove duplicates based on set_name, card_name, card_index, and rarity
-- Keeps the record with the smallest ID (earliest inserted)

DELETE FROM card_jp a USING (
    SELECT min(id) as id, set_name, card_name, card_index, rarity
    FROM card_jp 
    GROUP BY set_name, card_name, card_index, rarity
    HAVING count(*) > 1
) b 
WHERE a.set_name = b.set_name 
